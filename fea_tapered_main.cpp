...
MatrixXd fea_maint(MatrixXd Parameters, double A_1, double A_2, double Len, double t_tot, double F_max) {
	...
	int elements = 10;				//Number of elements in which problem is to be divided
	double force = 0;					
	int gauss_weights = 2;
	...
	tie(Areas, Lengths) = get_areas_lengths(elements, A_1, A_2, Len);	
	...
	while (abs(force)<= abs(F_max) ) {
		...
		force = force + (1 / double(steps_num)) * double(F_max);
	}
